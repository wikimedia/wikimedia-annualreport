...
function create_l2w(area, config) {
    config = config || {};

    langs = config.langs || ['en'];
    auto_width = config.auto_width || false;
    show_counter = config.show_counter || false;
    user_announcements = config.new_user_announcements || false;
    ...
    var available_langs = {
        ...
        'hu': 'ws://wikimon.hatnote.com:9320',
    ...
    for (var i = 0; i < langs.length; i++) {
        var lang = langs[i];
        var lang_url = available_langs[lang]
        connections[lang] = new wikipediaSocket.init(lang_url, lang, svg);
        if (auto_start) {
            connections[lang].connect();
        }
    }
    ...
    controls.close = function() {
        for (l in connections) {
            if (connections.hasOwnProperty(l)) {
                connections[l].close();
            }
        }
    }
